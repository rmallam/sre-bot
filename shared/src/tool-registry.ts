...
export const TOOL_REGISTRY: Record<ToolCallName, ToolDefinition> = {
  ...
  'cicd.fetch_run': {
    name: 'cicd.fetch_run',
    description: 'Fetch GitHub Actions workflow run and failed job logs',
    risk: 'low',
    requiresHilInProd: false,
    supportsDryRun: false,
    idempotent: true,
    maxRetries: 2,
    requiredFields: ['incidentId', 'githubRepo'],
    allowedModes: ['ci-failure'],
  },
  'cicd.rerun_workflow': {
    ...
    idempotent: false,
    maxRetries: 1,
    requiredFields: ['incidentId', 'githubRepo', 'workflowRunId'],
    allowedModes: ['ci-failure'],
  },
  'cicd.open_pr': {
    name: 'cicd.open_pr',
    description: 'Open CI workflow fix PR',
    risk: 'high',
    requiresHilInProd: true,
    supportsDryRun: false,
    idempotent: false,
    maxRetries: 1,
    requiredFields: ['incidentId', 'githubRepo', 'title', 'body'],
    allowedModes: ['ci-failure'],
  },
  'cicd.open_code_pr': {
    name: 'cicd.open_code_pr',
    description: 'Open PR with dependency/code file patches from CI triage',
    risk: 'high',
    requiresHilInProd: true,
    supportsDryRun: false,
    idempotent: false,
    maxRetries: 1,
    requiredFields: ['incidentId', 'githubRepo', 'title', 'body', 'patches'],
    allowedModes: ['ci-failure'],
  },
  'coding_agent.run_fix': {
    name: 'coding_agent.run_fix',
    description: 'Run multi-attempt LLM code fix loop and open PR when checks pass',
    risk: 'high',
    requiresHilInProd: true,
    supportsDryRun: false,
    idempotent: false,
    maxRetries: 1,
    requiredFields: ['incidentId', 'runId', 'ciRun'],
    allowedModes: ['ci-failure'],
  },
  'investigator.logs_query': {
    name: 'investigator.logs_query',
    description: 'Query pod logs or Loki for diagnostics',
    risk: 'low',
    requiresHilInProd: false,
    supportsDryRun: false,
    idempotent: true,
    ...
    allowedModes: ['diagnose', 'ci-failure'],
  },
  'investigator.metrics_query': {
    name: 'investigator.metrics_query',
    description: 'Query Prometheus or K8s metrics for deployment health',
    risk: 'low',
    requiresHilInProd: false,
    supportsDryRun: false,
    idempotent: true,
    maxRetries: 2,
    requiredFields: ['incidentId', 'namespace'],
    allowedModes: ['diagnose', 'ci-failure'],
  },
};
