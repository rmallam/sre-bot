...
for pair in \
  ...
  if e2e_curl -sf -m 10 "$url" >/dev/null 2>&1; then
    pass "$agent /health"
  else
...
ORCH_HEALTH="$(e2e_curl -sf -m 10 "$ORCHESTRATOR_URL/health" 2>/dev/null || echo '{}')"
...
CH="$(e2e_curl -sf -m 60 "$INVESTIGATOR_URL/cluster-health" 2>/dev/null || echo '{}')"
...
ACCEPT1="$(e2e_curl -sf -m 15 -X POST "$ORCHESTRATOR_URL/runs" -H 'Content-Type: application/json' -d "$RUN_BODY" 2>/dev/null || echo '{}')"
...
sleep 2
DEDUPE="$(e2e_curl -sf -m 15 -X POST "$ORCHESTRATOR_URL/runs" -H 'Content-Type: application/json' -d "$RUN_BODY" 2>/dev/null || echo '{}')"
...
for _ in 1 2 3 4 5 6; do
  RUNS="$(e2e_curl -sf -m 15 "$ORCHESTRATOR_URL/runs?incidentId=$E2E_ID&limit=5" 2>/dev/null || echo '{}')"
...
TOOLS="$(e2e_curl -sf -m 10 "$ORCHESTRATOR_URL/tools" 2>/dev/null || echo '{}')"
...
AM_RESP="$(e2e_curl -sf -m 20 -X POST "$COMMANDER_URL/webhooks/alertmanager" \
...
if echo "$AM_RESP" | python3 -c "import json,sys; d=json.load(sys.stdin); exit(0 if d.get('ok') is True else 1)" 2>/dev/null; then
  ...
  HTTP_CODE="$(e2e_curl -s -m 20 -o /dev/null -w '%{http_code}' -X POST "$COMMANDER_URL/webhooks/alertmanager" \
...
if e2e_curl -sf -m 10 "$PLATFORM_URL/health" >/dev/null 2>&1; then
  ROUTE="$(e2e_curl -sf -m 20 -X POST "$PLATFORM_URL/route" -H 'Content-Type: application/json' \
  ...
  RAG="$(e2e_curl -sf -m 20 -X POST "$PLATFORM_URL/rag/ground" -H 'Content-Type: application/json' \
...
DM_HEALTH="$(e2e_curl -sf -m 5 "$DEBUG_MCP_URL/health" 2>/dev/null || echo '')"
if [[ -n "$DM_HEALTH" ]]; then
  ...
  if echo "$DM_HEALTH" | python3 -c "import json,sys; d=json.load(sys.stdin); exit(0 if d.get('enabled') else 1)" 2>/dev/null; then
    TOOLS="$(e2e_curl -sf -m 10 "$DEBUG_MCP_URL/v1/tools" 2>/dev/null || echo '{}')"
...
AUTH_CFG="$(e2e_curl -sf -m 10 "$CONSOLE_URL/api/auth/config" 2>/dev/null || echo '{}')"
...
if e2e_curl -sf -m 10 "$CONSOLE_URL/api/runs?limit=5" >/dev/null 2>&1; then
  pass "GET /api/runs"
else
  RUNS_API="$(e2e_curl -sf -m 10 "$CONSOLE_URL/api/v1/runs?limit=5" 2>/dev/null || echo '')"
  if [[ -n "$RUNS_API" ]]; then
...
if [[ -n "$RUN_ID" ]]; then
  DETAIL="$(e2e_curl -sf -m 15 "$ORCHESTRATOR_URL/runs/$RUN_ID" 2>/dev/null || echo '{}')"
...
if ! e2e_curl -sf -m 5 "$SEC_URL/health" >/dev/null 2>&1 && [[ "$AUTO_PORT_FORWARD" == "true" ]]; then
  start_port_forward security-agent 9088
  sleep 2
fi
if e2e_curl -sf -m 5 "$SEC_URL/health" >/dev/null 2>&1; then
  SAN="$(e2e_curl -sf -m 15 -X POST "$SEC_URL/sanitize-for-llm" -H 'Content-Type: application/json' \
...
OBS_LOG="$(e2e_curl -sf -m 20 -X POST "$INVESTIGATOR_URL/observability/logs" -H 'Content-Type: application/json' \
...
SKILLS="$(e2e_curl -sf -m 15 "$ORCHESTRATOR_URL/runs/skills-export?limit=20" 2>/dev/null || echo '{}')"
assert_json_truthy "skills-export returns markdown" "$SKILLS" "bool(d.get('markdown'))"
GROUPED="$(e2e_curl -sf -m 15 "$ORCHESTRATOR_URL/runs/by-resource?limit=20" 2>/dev/null || echo '{}')"
...
CACHE_ACCEPT="$(e2e_curl -sf -m 15 -X POST "$ORCHESTRATOR_URL/runs" -H 'Content-Type: application/json' -d "$CACHE_BODY" 2>/dev/null || echo '{}')"
